...
const Page: PageEl = (props: WindowFloatPropsType & { [key: string]: any },
  ...
  return <>
    ...
    <div id={hash} className={z.qestyles.dialog + " p-0 scale-50 opacity-0 transition-all duration-500"}
      style={{
        width: "95vw",
      ...
      <f-csb class={z.qestyles.dialogtitle + " " + z.qestyles.title} style={{ height: 30, fontSize: 12, }}>

        <f-cc>
          <f-cc className={z.qestyles.dialogbtn + (z.lang.dir == "rtl" ? " rounded-tr-md" : " rounded-tl-md") + " cursor-pointer"} style={{ width: 40, height: 30 }}
            onClick={() => props.onhelp?.()}>
            <InfoOutlineIcon style={{ color: "white", fontSize: 15 }} />
          </f-cc>
          {props.onsettings ? <f-cc className={z.qestyles.dialogbtn + " cursor-pointer"} style={{ width: 50, height: 30 }}
        ...
        <f-c style={{ direction: z.lang.dir, width: "100%", flex: 1 }}><span style={{ color: "white" }}><sp-3 />{props.title}<sp-2 /></span></f-c>

        <f-cc>

          <f-cc className={z.qestyles.dialogbtn + " cursor-pointer"} style={{ width: 40, height: 30 }}
            onClick={() => {
              if (props["maxw"] == "95vw") {
                props["maxw"] = null;
                props["minh"] = null;
                ...
                props["maxw"] = "95vw"
                if (props.maximizeHeight)
                  props["minh"] = "70vh";
              }
              refresh()
            }}>
            {props["maxw"] == "95vw" ? <ContentCopyIcon style={{ color: "white", fontSize: 13, transform: "scaleX(-1)" }} /> : <CropFreeIcon style={{ color: "white", fontSize: 15 }} />}
          </f-cc>

          <f-cc className={z.qestyles.dialogbtn + (z.lang.dir == "rtl" ? " rounded-tl-md" : " rounded-tr-md") + " cursor-pointer"} style={{ width: 50, height: 30 }}
            onClick={async () => {
              setTimeout(() => {
                props.onclose?.()
              }, 400);
      ...
      <div style={{
        ...
        position: "relative", width: "100%", textAlign: "justify",
        minHeight: props["minh"] ? `calc(${props["minh"]} - 30px)` : null,
        padding: props.padding || "0.5rem 0.5rem 0.5rem 0.5rem", lineHeight: 1.5,
        borderBottomLeftRadius: 5, borderBottomRightRadius: 5, direction: z.lang.dir,
