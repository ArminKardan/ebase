...
export default (props: {
    ...
    if (textstring) {
        text = <h5 className={z.qestyles.text} style={{ fontSize: props.vfontSize || props.fontSize, fontWeight: props.fontWeight }}
            onClick={() => { props.on ? props.on() : null }}>{text}</h5>
    }

    var wmb = 0
    var wlink = props.wlink ? <span style={{ padding: "0 5px", borderRadius: 2 }}> {props.wlink} </span> : ""
    if (props.whighlight)
        wlink = props.wlink ? <span className={z.qestyles.linkbg} style={{ padding: "0 5px", borderRadius: 2 }}> {props.wlink} </span> : ""
    ...
    return <f-c class={z.qestyles.itemalign} style={{ fontSize: props.fontSize, margin: "2px 0" }}>
        {props.title ? <span className={z.qestyles.text} style={{
            width: props.nospace ? null : z.lang.textw,
        ...
        {props.image && typeof props.image == "string" ? <Img style={{
            ...
            src={props.image || props.icon}
            on={() => { props.onimage?.() }} /> : null}
        ...
        {props.extlink ? <><sp-4 />
            <icon style={{ display: "inline" }} class={'w-[9px] h-[9px] icon-[oi--external-link] ' + z.qestyles.title} />

            {/* <Img src={global.cdn("/files/extlink.svg")} style={{
        ...
        {copy ? <><sp-2 /><Img className={z.qestyles.menubtn}
            style={{ width: 15, height: 15, cursor: "pointer" }} src={state.copied ? global.cdn("/files/ok.svg") : global.cdn("/files/copy.svg")}
            on={() => { Copy(copy); setState({ ...state, copied: true }); setTimeout(() => { setState({ ...state, copied: false }) }, 3000) }} /></> : null}
        <sp-3 />{wlink}

        {props.help || props.onhelp ? <><sp-3 /><Img src={global.cdn("/files/helpg.svg")}
            style={{ display: "inline-block", verticalAlign: "middle", width: 14, height: 14, opacity: 0.5, cursor: "pointer" }}
            on={() => { props.onhelp?.() }} /></> : null}
