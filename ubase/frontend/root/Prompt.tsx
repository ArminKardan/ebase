...
export default (props) => {
    ...
    window.closelog = () => {
        if (Object.keys(global.promptstate).some(k => k.startsWith("customer")))
    ...
    window.login = () => {
        setState({ ...global.promptstate, show: "login" })
    }
    ...
    // }
        ...
        dialog = <TextAreaEditFloat title={global.promptstate.title || z.lang.sysmsg} title2={global.promptstate.text} maxlen={global.promptstate.maxlen}
            ...
            }} on={(txt) => { window["promptresolve"](txt); setState({ ...global.promptstate, show: false }) }} />
    }
        ...
        dialog = <WindowFloat
            ...
            <div style={{ maxHeight: 400, overflowX: "scroll" }}>
                {(global.promptstate.items).filter(st => (!global.promptstate.search) || (st.title1 || "")
                    .includes(global.promptstate.search) || (st.title2 || "").includes(global.promptstate.search))
                    .map(st => {
                        ...
                        return <Icon2Titles title1={st.title1}
                            class={st.highlight ? z.styles.bgselected : z.styles.bg2}
                            style={{ marginBottom: 1 }}
                            icon={image}
                            title2={<f-9 style={{ marginTop: 5 }}>{st.title2}</f-9>}
                            righticon={rightic}
                            on={async () => {
        ...
        dialog = <WindowFloat
            ...
            <TextBox selectonclick defaultValue={global.promptstate.search} on={txt => {
                clearTimeout(global.cpickerprompt)
                global.cpickerprompt = setTimeout(() => {
                    setState({ ...global.promptstate, search: txt || "" })
                }, 100);

            }} placeholder={z.lang.search} style={{ fontSize: 13 }} />
            <br-x />

            <div style={{ maxHeight: 400, overflowX: "scroll" }}>
                {(items).filter(st => (!global.promptstate.search) || (st.title1 || "").
                    toLowerCase().trim().includes((global.promptstate.search || "").toLowerCase().trim())
                    || (st.title2 || "").toLowerCase().trim().includes((global.promptstate.search || "").toLowerCase().trim()))
                    (items).map(st => {
        ...
        dialog = <WindowFloat
            ...
            <f-cc class={!global.promptstate.percent ? z.qestyles.none : z.qestyles.op1}>
                <Upload
                    ...
                    singlefile
                    // hidefileicons
                    onclear={() => setState({ ...global.promptstate, percent: null, url: null, maxWidth: null })}
                    on={(url) => {
        ...
        dialog = <LogFloat function={window["logger"]} onclose={() => { setState({ ...global.promptstate, show: false }) }} z={1000} />
    }
        ...
        >
            {global.promptstate.json ? <pre style={{ fontSize: 11, direction: "ltr", maxHeight: "70vh", overflowY: "scroll" }}>
                ...
                }} src={global.promptstate.watermark} /> : null}
                ...
                <div style={{ zIndex: 200, width: "100%" }}>
                    ...
                    <br-x />
                    <br-xx />
                    <b-200 class={z.styles.btnaccept}
